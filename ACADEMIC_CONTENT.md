...
互动实验使用 24 个固定的合成人物和两个独立控制量，只解释“人设匹配与额外行为维度上的多样性需要分别评价”。不展示论文实验分数，不调用模型。网址中的 `labf` / `labd` 可分享状态；论文页的 `q` / `topic` / `status` 可分享检索条件。关闭 JavaScript 后保留全部论文、研究内容及实验的静态说明。检查交互逻辑可运行 `node scripts/test_research.js`，内容同步回归检查运行 `python scripts/test_academic.py`。
