...
`data/publication-tags.json` maintains website-only topic and application tags, separately from the CV facts. Assign each paper a small set of directly supported tags (usually 1–4), using its stable publication key. The Publications filter groups applications and topics/methods separately, and clicking a paper tag opens that filtered list. Search also includes the visible tag labels. The original broad research-theme filters and URLs remain supported; use distinct slugs for granular tags (for example `social-simulation-methods`) to avoid changing their meaning. Rebuild with `python scripts/build_academic.py` after editing tags.
